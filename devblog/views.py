...
def ajax_get_more_stories(request, story_offset, num_stories):
    # TODO: this can be handled in the urls.py, check documentation
    story_offset = int(story_offset)
    num_stories = int(num_stories)
    context = {'stories': Story.objects.order_by('-date')
               [story_offset:story_offset+num_stories]}
